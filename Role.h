...
public:
	Role(std::string, std::vector<UserRoleRelation>&, std::vector<STZone>&, std::vector<RoleHierarchy>&,
		std::vector<RoleHierarchy>&, std::vector<SOD>&, std::vector<PermissionAssignment>&);
	...
	const PermissionAssignment& assignPermission(const Permission& p, const STZone& z);
...
private:
	...
	enum class RHType { A_HIERARCHY = 1, I_HIERARCHY }; //enumaration for createRoleHierarchy
	RoleHierarchy* createRoleHierarchy(RHType, Role&, STZone&);

	enum class SoDType { SSoD = 1, DSoD }; //enumeration for createSoD
	SOD* createSoD(SoDType, Role&, STZone&);
